...
def align_xrr_height(  rang = 0.3, point = 31   ):             
...
def alignement_xrr(angle = 0.15):      
        ...
        smi = SMI_Beamline()
        yield from smi.modeAlignment_xrr()
        
        #Set direct beam ROI
        yield from smi.setDirectBeamROI()

        #Scan theta and height
        yield from align_xrr_height(700, 16, der=True)
        yield from align_xrr_prs(0.7, 11)
        yield from align_xrr_height(300, 11, der=True)
        yield from align_xrr_prs(0.4, 16)
        
        #move to theta 0 + value
        yield from bps.mv(prs, ps.peak + angle)

        #Set reflected ROI
        yield from smi.setReflectedBeamROI_xrr(total_angle=angle)
...
def alignement_gisaxs(angle = 0.15):      
        ...
        yield from align_gisaxs_th(0.7, 11)
        yield from align_gisaxs_height(300, 11, der=True)
        yield from align_gisaxs_th(0.4, 16)
...
def alignement_gisaxs_hex_short(angle = 0.12):      
        ...
        #Scan theta and height
        yield from align_gisaxs_height_hex(0.300, 16, der=True)
        #yield from align_gisaxs_th_hex(1, 21)
        #yield from align_gisaxs_height_hex(0.300, 11, der=True)
        #yield from align_gisaxs_th_hex(0.4, 16)
        
        #move to theta 0 + value
        #yield from bps.mv(stage.th, -0.63 + angle)
