...
def expert_gisaxs_scan(dets = [pil300KW, pil1M],
    ...
    if 'piezo' in [motor_names]: all_detectors.append(piezo)
    else: sd.baseline.append(piezo)

    if 'stage' in [motor_names]: all_detectors.append(stage)
    else: sd.baseline.append(stage)

    if 'prs' in [motor_names]: all_detectors.append(prs)
    else: sd.baseline.append(prs)
   
    if 'energy' in [motor_names]: all_detectors.append(energy)
    ...
    assert(len(motor_names) == len(motor_range), f'Number of motor range ({len(motor_range)}) is different from number of motors ({len(motor_names)})')
    motor_dic = {}
    ...
    if len(sam_name) != 1 and '.x' not in motor_names:
        ...
        motor_name = [motor for motor in motor_names if '.x' in motor][0]
        assert (len(sam_name) == list(motor_dic[motor_name])[2], f'Number of sample({len(sam_name)}) is different from the ({list(motor_dic[motor_name])[2]}) x position given')
...
def gisaxs_scan(dets=[pil300KW, pil1M],
    ...
    print('opening the run')

    yield from bps.open_run(md=base_md)
    yield from bp.scan(all_detectors, motor_names, *motor_range)
