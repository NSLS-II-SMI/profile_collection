...
#             md_return[f"ROI{roi_num}_X_min"] = getattr(
#                 self.detector, "min_xyz"
#             ).min_x.value
#             md_return[f"ROI{roi_num}_Y_min"] = getattr(
#                 self.detector, "min_xyz"
#             ).min_y.value
...
#         if False:
#             self.fs1 = DiagnosticScreen(
#                 "fs1",
#                 27.2,
#                 pv="XF:11BMA-BI{FS:1}",
#                 epics_signal=StandardProsilica("XF:11BMA-BI{FS:1-Cam:1}", name="fs1"),
#             )
...
#             )
#             self.fs5 = DiagnosticScreen(
#                 "fs5",
#                 70.0,
#                 pv="XF:11BMB-BI{FS:Test-Cam:1}",
#                 epics_signal=StandardProsilica("XF:11BMB-BI{FS:4-Cam:1}", name="fs5"),
#             )
#         else:
#             # Rely on the fact that these are defined in 20-area-detectors.py
#             self.fs1 = DiagnosticScreen(
#                 "fs1", 27.2, pv="XF:11BMA-BI{FS:1}", epics_signal=fs1
#             )
...
#             )
#             self.fs5 = DiagnosticScreen(
#                 "fs5", 70.0, pv="XF:11BMB-BI{FS:Test-Cam:1}", epics_signal=fs5
#             )
...
#         update: 090517, RL: change the wait_time from 0.005 to 0.1, change sleep to yield from bps.sleep"""
...
#                 while (not self.is_on(verbosity=0)) and (
#                     time.time() - start_time
#                 ) < retry_time:
#                     if verbosity >= 5:
#                         print(
#                             "  try {:d}, t = {:02.2f} s, state = {:s}".format(
#                                 itry + 1,
#                                 (time.time() - start_time),
#                                 "OPEN_____" if self.is_on(verbosity=0) else "CLOSE====",
#                             )
#                         )
#                     yield from bps.sleep(poling_period)
...
#                 start_time = time.time()
#                 while (
#                     self.is_on(verbosity=0) and (time.time() - start_time) < retry_time
#                 ):
#                     if verbosity >= 5:
#                         print(
#                             "  try {:d}, t = {:02.2f} s, state = {:s}".format(
...
#                 while (not self.blade1_is_on(verbosity=0)) and (
#                     time.time() - start_time
#                 ) < retry_time:
#                     if verbosity >= 5:
#                         print(
...
#             yield from bps.sleep(1.0)  # Wait for filter box to settle
...
#                     and self._chamber_pressure_pv.get() < range_low
#                 ):
#                     monitor = False

#                 if (
#                     range_high is not None
#                     and self._chamber_pressure_pv.get() > range_high
#                 ):
#                     monitor = False
...
#                 yield from bps.sleep(readout_period)
...
#         tries = 1
#         while (
#             caget("XF:11BMB-VA{Chm:Det-Pmp:1}Sts:Enbl-Sts") == 0 and tries <= max_tries
#         ):
