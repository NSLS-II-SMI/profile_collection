...
def Rb_tswaxs(t=0.5, waxs_arc = [13],energies=[15150,15160,15170,15180,15190,15191,15192,15193,15194,15195,15196,15197,15198,15199,15200,15201,15202,15203,15204,15205,15210,15215,15220,15230,15240,15250]):
    #holder 1 
...
#def giwaxsTempSingleWaxsSeries(x_list,y_list,th_list,chi_list,sample_list,waxs_arc,num,t=1,user='BP'):
    ...
    sample_list = ['6SS_A','6SH_A','7SS_A','7SS_B','8SH_A','10SS_B','10SH_A','6CS_B','10CS_B','airblank']
    ...
    for waxspos in waxs_arc:
        for x, y, sample in zip(x_list,y_list,sample_list): #loop over samples on bar
            ...
            yield from bps.mv(waxs,waxspos) #move the waxs dectector to the measurement position
            waxs_arc = [waxspos]
           ...
           # yield from bp.scan(dets, waxs.arc, *waxs_arc)# should just be a single point "scan"
            ...
            for e in energies:                              
                ...
                sample_name = '{sample}_saxs2m_waxs{waxspos:5.4f}_en{energy}'.format(sample = sample, waxspos=waxspos,energy=e)
...
def giwaxs_S_edge(t=1):
    ...
    names = ['P3HT_CF','P3HT_blend3','P3HT_600','P3HT_blend2','P3MEEMT_75','P3MEEMT_175','P3HHT_A10','P3HHT_B8','BHJ_90_AS','BHJ_90_AN']
...
def giwaxsTempSingleWaxsSeries(x_list,y_list,th_list,chi_list,sample_list,waxs_arc,num,t=1,user='BP'):
    print(num)
    for waxspos in waxs_arc:
        for x, y, th, chi, sample in zip(x_list,y_list,th_list,chi_list,sample_list): #loop over samples on bar
            yield from bps.mv(piezo.x, x) #move to next sample
            yield from bps.mv(piezo.y, y) #move to next sample
            yield from bps.mv(piezo.th, th) #move to next sample
            yield from bps.mv(piezo.ch, chi) #move to next sample
...
def heatingLoop():
    ...
    while counter<nscans:
        for xmv in xmod:
            lclxpos = xpos + xmv
            yield from giwaxsTempSingleWaxsSeries(lclxpos,ypos,thpos,chipos,names,[9.5,3],counter,t=1,user='PB1-1')
            sleep(sleepbetweenexps)
            counter+=1
            yield from giwaxsTempSingleWaxsSeries(lclxpos,ypos,thpos,chipos,names,[3,9.5],counter,t=1,user='PB1-1')
            sleep(sleepbetweenexps)
            counter+=1    
