...
# 3AM to 7AM
sample_dict = {  1: 'Fe3O4-LC3-0.1-5CB' ,
 2: 'Fe3O4-LC3-0.01-5CB',  3: 'LC1S-1-5CB', 
 4: 'LC2S-1-5CB',5: 'LC3S-1-5CB', 
 6: 'LC2P-1-5CB',7: 'LC3P-1-5CB',
 8: 'Au-LC1-1-5CB', 9:'Au-LC2-1-5CB', 
 10: 'Au-LC3-1-5CB', 11: 'NR1-LC2-1-5CB',
 12: 'NR1-LC3-1-5CB', 13: 'NR2-LC2-1-5CB',
 14: 'NR2-LC3-1-5CB',
...
def run_T_wsaxs(   x_list=x_list, sample_list=sample_list,   t=1, username =  username, 
         waxs_angles = [ 0, 15   ] , Tlist= [ 30, 31, 32, 32.5, 33, 33.5, 34, 34.5, 35, 35.5, 36, 36.5, 37 , 37.5,  38, 38.5,   39, 39.5,  40, 40.5, 41, 41.5, 42, 43, 44, 45, 44, 43, 42, 41, 40.5, 40, 39.5, 39, 38.5, 38, 37.5, 37, 36.5, 36, 35.5, 35, 34.5, 34, 33.5, 33, 32  ], 
         saxs_on=True, ): 
    ...
    for (cts, T)  in enumerate( Tlist ):
        ...
        yield from  measure_series_multi_angle_wsaxs_yield(  t= [t], waxs_angles= waxs_angles,  dys = [ 0 ]  ) 
...
def run_T_giwsaxs(   x_list=x_list, sample_list=sample_list,   t=1, username =  username, 
         inc_angles=[  0.05,  0.12, 0.15,  0.18,  0.3 ], waxs_angles = [ 15   ] ,
         x_shift_array = [ -500,   0,    500 ]  ,  
             Tlist= [ 30 , 40 , 50 , 60, 65, 70, 75, 80, 85, 90, 100, 120, 100, 90, 85, 80, 75, 70, 65, 60, 30], 
         saxs_on=True,): 
    ...
    for (cts, T)  in enumerate( Tlist ):
        ...
        yield from run_giwsaxs( x_list=x_list, sample_list=sample_list,  t=t, username =  username, inc_angles=inc_angles,
              waxs_angles =  waxs_angles ,  x_shift_array =  x_shift_array , saxs_on=True, align = align,
               YPOS = YPOS ,  ThPOS =   ThPOS , T=T  )
...
def run_giwsaxs( x_list=x_list, sample_list=sample_list,   t=1, username =  username, 
  inc_angles=[  0.05, 0.15, 0.3 ], waxs_angles = [ 0, 20   ] ,
  x_shift_array = [ 0 ]  ,     YPOS = {   },  ThPOS = {   },
...
def measure_series_multi_angle_wsaxs_yield(  t= [  1 ] , waxs_angles= [ 0, 15 ] , 
 dys = [ 0,  -2000,  -2000 , -2000, -2000]   ):
...
def measure_series_multi_angle_wsaxs(  t= [  1 ] , waxs_angles= [ 0, 15 ] , 
 dys = [ 0,  -2000,  -2000 , -2000, -2000]   ):
...
def measure_series_wsaxs(  t= [  1 ] , waxs_angle=15,  dys = [ 0,  -1000,  -2000 , -3000, -4000, -6000, -8000, -10000 ]   ):
    ks = list( sample_dict.keys() )  #[:8 ]   
    for k in ks:
        mov_sam( k )                   
        for dy in dys: 
