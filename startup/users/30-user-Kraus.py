...
def run_saxs_kraus_1(t=1): 
    ...
    # Detectors, motors:
    dets = [pil1M,pil300KW,rayonix]
    y_range = [[-4000, 4000, 161],[-3500, 3500, 141], [-3000, 3000, 121], [-3000, 3000, 121], [-3000, 3000, 121]]
    x_range = [[-4000, 4000, 33],[-3500, 3500, 29], [-3000, 3000, 25], [-3000, 3000, 25], [-3000, 3000, 25]] #[2.64, 8.64, 2]
    samples = [  '67-N_0p05', '76-4core_0p05', '76-6core_0p05', '76-8core_0p05','76-10core_0p05']
    #    param   = '16.1keV'
    assert len(x_list) == len(samples), f'Number of X coordinates ({len(x_list)}) is different from number of samples ({len(samples)})'
    det_exposure_time(t)
    yield from bps.mv(piezo.z, 5000)
...
def run_saxs_kraus_2(t=1): 
    ...
    # Detectors, motors:
    dets = [pil1M,pil300KW,rayonix]
    y_range = [[-2500, 2500, 101], [-2500, 2500, 101], [-2500, 2500, 101], [-2500, 2500, 101],[-2500, 2500, 67]]
    x_range = [[-2500, 2500, 21], [-2500, 2500, 21], [-2500, 2500, 21], [-2500, 2500, 21],[-2500, 2500, 21]] #[2.64, 8.64, 2]
    samples = [ '76-8core_0p025_c', '76-6core_0p025_c', '76-4core_0p025_c','67-N_0p025_c','76-10core_0p15']
    #    param   = '16.1keV'
    assert len(x_list) == len(samples), f'Number of X coordinates ({len(x_list)}) is different from number of samples ({len(samples)})'
    det_exposure_time(t)
    yield from bps.mv(piezo.z, 5000)
...
def run_saxs_kraus_3(t=1): 
    ...
    # Detectors, motors:
    dets = [pil1M,pil300KW,rayonix]
    y_range = [[-2500, 2500, 67], [-2500, 2500, 67], [-2000, 2000, 54], [-2000, 2000, 54], [-2500, 2500, 51], [-2000, 2000, 41], [-2000, 2000, 41] ]
    x_range = [[-2500, 2500, 21],  [-2500, 2500, 21] , [-2000, 2000, 17], [-2000, 2000, 17], [-2500, 2500, 21], [-2000, 2000, 17], [-2000, 2000, 17]] #[2.64, 8.64, 2]
    samples = [  '76-8core_0p15', '76-6core_0p15', '76-4core_0p15', '67-N_0p15','76-10core_0p25','76-8core_0p25','76-6core_0p25']
    #    param   = '16.1keV'
    assert len(x_list) == len(samples), f'Number of X coordinates ({len(x_list)}) is different from number of samples ({len(samples)})'
    det_exposure_time(t)
    yield from bps.mv(piezo.z, 5000)
...
def run_saxs_kraus_4(t=1): 
    ...
    # Detectors, motors:
    dets = [pil1M,pil300KW,rayonix]
    y_range = [ [-2500,2500,51],[-2500,2500,51], [-2500,2500,51],[-2500,2500,51],[-2500,2500,51],[-2500,2500,51]]
    x_range = [[-2500,2500, 21], [-2500,2500, 21], [-2500,2500, 21], [-2500,2500, 21], [-2500,2500, 21], [-2500,2500, 21]] #[2.64, 8.64, 2]
    samples = ['76-4core_0p25','67-N_0p25','76-10core_0p50','76-8core_0p50','76-6core_0p50','76-4core_0p50']
    #    param   = '16.1keV'
    assert len(x_list) == len(samples), f'Number of X coordinates ({len(x_list)}) is different from number of samples ({len(samples)})'
    det_exposure_time(t)
    yield from bps.mv(piezo.z, 5000)
...
def run_saxs_kraus_5(t=1): 
    ...
    # Detectors, motors:
    dets = [pil1M,pil300KW,rayonix]
    y_range = [[-3500,3500,71],[-3500,3500,11], [-3500,3500,11],[-3500,3500,11],[-3500,3500,11],[-3500,3500,11]]
    x_range = [[-3500,3500,29], [-3500,3500,11], [-3500,3500,11], [-3500,3500,11], [-3500,3500,11], [-3500,3500,11]] #[2.64, 8.64, 2]
    samples = ['67-N_0p5','76-10core_tkp','76-8core_tkp','76-6core_tkp','76-4core_tkp','67-N_tkp']
    #    param   = '16.1keV'
    assert len(x_list) == len(samples), f'Number of X coordinates ({len(x_list)}) is different from number of samples ({len(samples)})'
    det_exposure_time(t)
    yield from bps.mv(piezo.z, 5000)
...
def run_saxs_kraus_RZA2(t=1): 
    ...
    x_range = [[-2000,2000,4],[-5000,5000,41],[-5000,5000,41]] #[2.64, 8.64, 2]
...
def run_saxs_kraus_micro(t=0.5): 
    ...
    x_range = [[-750,750,41],[-750,750,41],[-750,750,41],[-750,750,41]] #[2.64, 8.64, 2]
