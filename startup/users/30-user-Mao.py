...
sample_dict = {   9: 'AgBH'}
pxy_dict = {  4: (  24.9, 0.5   ),  5:  ( 18.6, 0.5  ),  6:  ( 12.2, 0.5  ),  7:  ( 5.8, 0.5  ) ,8:  ( -0.6, 0.5  ),
9:  ( -6.8, 0.5 ),10:  ( -13.1, 0.5 ),11:  (    -19.5, 0.5  ), }
...
42%         3   3
    ...
    h_dict = { 0: [0,4],  20: [0,0], 42: [ 3,3  ],  50: [ 3, 2.7  ], 60: [ 3, 2.53  ], 70: [ 3,2.42 ], 80: [ 3, 2.33  ],  100: [ 5, 0  ]  }
    if h not in available_h:
        print('This humidity: %s is not available. The availalbe are: %s'%(h, available_h)) 
    else:
        wet_old = moxa_in.ch2_sp.get()
        dry_old = moxa_in.ch1_sp.get()
        ch = readHumidity()
...
def _measure_one(  t0, ks, dets, waxs_angle, dx=0, dy=0 ,  t=0.5   ):
    for k in ks:
        ...
        x_pos=stage.x.position, y_pos=stage.y.position, z_pos=stage.z.position,
                    tx = np.round((tcur - t0), 1), h= np.round( readHumidity(temperature=25, voltage_supply=5, verbosity=1), 2), 
...
def measure_saxs( t = 1, att='None', dy=0, user_name='YM', sample= None ): 
    ...
    sample_name = name_fmt.format(sample=sample, x_pos=np.round(stage.x.position,2), y_pos=np.round(stage.y.position,2),
...
def measure_waxs( t = 1, waxs_angle=0, att='None', dy=0, user_name='YM', sample= None ): 
    ...
    sample_name = name_fmt.format(sample=sample, x_pos=stage.x.position, y_pos=stage.y.position, z_pos=stage.z.position,
...
def measure_waxs_multi_angles( t = 1.0, att='None', dy=0, user_name='YM',  
