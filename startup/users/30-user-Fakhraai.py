...
def grazing_Luo_2022_2(t=0.5, incident_angle=0.1):
    ...
    assert len(names) == len(piezo_xs), f'Number of X coordinates ({len(piezo_xs)}) is different from number of samples ({len(names)})'
    assert len(hexa_xs) == len(names), f'Number of X hexapod coordinates ({len(hexa_xs)}) is different from number of samples ({len(names)})'
    ...
    for name, piezo_x, hexa_x in zip(names[skip:], piezo_xs[skip:], hexa_xs[skip:]):
        ...
        for i, wa in enumerate(angles):
            ...
            sample_name = name_fmt.format(sample=name, energy='%.1f'%e, sdd='%.0f'%sdd, wax=wa, ai='%.1f'%incident_angle, xbpm='%4.3f'%bpm)
...
def grazing_gradient_Luo_2022_2(t=0.5, incident_angle=0.1):
    ...
    assert len(names) == len(sample_edges), f'Number of samples ({len(names)}) is different from number of edges ({len(sample_edges)})'
    assert len(names) == len(sample_margins), f'Number of samples ({len(names)}) is different from number of margins ({len(sample_margins)})'
    ...
    for name, edges, margins in zip(names, sample_edges, sample_margins):
        ...
        positions = [1 + i * step_across_gradient for i in range(100) if i * 2 < length / 1000]
        ...
        for i, position in enumerate(positions):
            ...
            for wa in angles:
                ...
                # Metadata
                name_fmt = '{sample}_{energy}keV_{sdd}m_wa{wax}_ai{ai}_pos_{pos}mm_bpm{xbpm}'
                bpm = xbpm2.sumX.get()
                e = energy.energy.position / 1000
                sdd = pil1m_pos.z.position / 1000

                sample_name = name_fmt.format(sample=name, energy='%.1f'%e, sdd='%.0f'%sdd, wax=wa, ai='%.1f'%incident_angle,
                                              pos='%.1f'%position, xbpm='%4.3f'%bpm)
                sample_id(user_name=user_name, sample_name=sample_name)
