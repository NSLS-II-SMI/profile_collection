...
def mapping_saxs_Greer(t=5): 
    ...
    assert len(x_list) == len(y_list), f'Number of X coordinates ({len(x_list)}) is different from Y coordinates ({len(y_list)})'
    assert len(x_list) == len(x_range), f'Number of X coordinates ({len(x_list)}) is different X ranges ({len(x_range)})'
    assert len(x_list) == len(y_range), f'Number of X coordinates ({len(x_list)}) is different Y ranges ({len(y_range)})'
