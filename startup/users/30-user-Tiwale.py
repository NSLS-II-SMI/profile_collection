...
def ex_situ_nexafsznedge_2021_2(t=1):
    ...
    energies = np.arange(9640, 9660, 5).tolist() + np.arange(9660, 9680, 0.5).tolist() + np.arange(9680, 9700, 1).tolist() + np.arange(9700, 9720, 2).tolist() + np.arange(9720, 9750, 2).tolist()


    samples =  [ 'SnZ_redo',  'SbZ',  'SPZ',   'PZ', 'Sn_exp_Z', 'Sb_exp_Z', 'SP_exp_Z', 'PZ_exp']
...
def ex_situ_saxsnexafsznedge_2021_2(t=1):
    ...
    energies = np.arange(9640, 9660, 5).tolist() + np.arange(9660, 9680, 0.5).tolist() + np.arange(9680, 9700, 1).tolist() + np.arange(9700, 9720, 2).tolist()
...
def night_nikhil(t=0.5):
    ...
    dets = [fd]
    det_exposure_time(t,t)
    yield from bp.rel_grid_scan(dets, piezo.y, *y_range, piezo.x, *x_range, 1) #1 = snake, 0 = not-snake
