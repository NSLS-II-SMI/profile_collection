...
def set_platform_0():
    RE.md['platform 0']=[printer.y_bed.user_readback.value,stage.y.user_readback.value]
...
def start_acq_long(dt=0.1, name='test', sleep=0):
    ...
    for i in range(500):
        ...
        sample_name = name_fmt.format(sample=name, td=td, energy='%.1f'%e, wax=wa, scan_id=scan_id)
        sample_name = sample_name.translate({ord(c): '_' for c in '!@#$%^&*{}:/<>?\|`~+ =,'})
        sample_id(user_name=user_name, sample_name=sample_name)
