...
def alignement_herzig():
    ...
    for name, xs_piezo, ys_piezo, zs_piezo, xs_hexa, ys_hexa in zip(names, x_piezo, y_piezo, z_piezo, x_hexa, y_hexa):
        yield from bps.mv(piezo.x, xs_piezo)
        yield from bps.mv(piezo.y, ys_piezo)
        yield from bps.mv(stage.y, ys_hexa)
        yield from bps.mv(stage.x, xs_hexa)
...
def S_edge_measurments_Herzig(t=1):
    ...
    for name, xs, zs, aiss, ys, xs_hexa, ys_hexa in zip(names, x_piezo, z_piezo, incident_angles, y_piezo_aligned, x_hexa, y_hexa):
        ...
        for i, xxx in enumerate(np.round(xssss, 2)):
            ...
            bpm = xbpm2.sumX.value
            sample_name = name_fmt.format(sample=name, energy='%6.2f'%2450.0, ai ='%3.2f'%0.8, pos = '%2.2d'%i)
            sample_id(user_name='LR', sample_name=sample_name)
            yield from bp.count(dets, num=1)

        energies = [2450.0, 2460.0, 2470.0, 2472.0, 2474.0, 2474.5, 2475.0, 2475.5, 2476.0, 2476.5, 2477.0, 2477.5, 2478.0, 2478.5,
        2479.0, 2479.5, 2480.0, 2481.0, 2482.0, 2483.0, 2484.0, 2485.0, 2486.0, 2487.0, 2490.0, 2500.0]
        det_exposure_time(t, t)     
        dets = [pil1M, pil300KW]
...
def run_Herzi_Sedge_2021_1(t=1):
    ...
    samples = ['020ac', '020oa', '050ac', '050oa', '024ac', '024oa', '010an', '009', '048an', '057an']
    ...
    for w, (xs, y, x_hexa, name) in enumerate(zip(x_piezo, y_piezo, x_hexa, samples)):
        ...
        for i, xxx in enumerate(np.round(xss, 2)):
            yield from bps.mv(piezo.x, xxx)
            bpm = xbpm2.sumX.value
            sample_name = name_fmt.format(sample=name, energy='%6.2f'%2450.0, ai ='%3.2f'%0.5, pos = '%2.2d'%i)
            sample_id(user_name='LR', sample_name=sample_name)
            yield from bp.count(dets, num=1)
...
def run_Herzi_2021_1(t=1): 
    ...
    for x, y, x_hexa, name in zip(x_piezo, y_piezo, x_hexa, samples):
        ...
        for wa in wa_ran:
            ...
            yield from bps.mv(piezo.x, x + 500)
            sample_name = name_fmt.format(sample=name, pos = 'pos1', angle='%3.2f'%0.14, wax = '%2.1f'%wa)
            sample_id(user_name='EH', sample_name=sample_name)
            print(f'\n\t=== Sample: {sample_name} ===\n')
            yield from bp.count(dets, num=20)

            yield from bps.mv(piezo.x, x - 500)
            sample_name = name_fmt.format(sample=name, pos = 'pos2', angle='%3.2f'%0.14, wax = '%2.1f'%wa)
            sample_id(user_name='EH', sample_name=sample_name)
            print(f'\n\t=== Sample: {sample_name} ===\n')
            yield from bp.count(dets, num=20)

            yield from bps.mv(piezo.x, x - 1000)
            sample_name = name_fmt.format(sample=name, pos = 'pos3', angle='%3.2f'%0.14, wax = '%2.1f'%wa)
            sample_id(user_name='EH', sample_name=sample_name)
            print(f'\n\t=== Sample: {sample_name} ===\n')
            yield from bp.count(dets, num=20)
...
def run_test_Herzi_2021_1(t=1): 
    ...
    for x, y, x_hexa, name in zip(x_piezo, y_piezo, x_hexa, samples):
        ...
        for wa in wa_ran:
            ...
            sample_name = name_fmt.format(sample=name, pos = 'pos4', angle='%3.2f'%0.14, wax = '%2.1f'%wa)
            sample_id(user_name='EH', sample_name=sample_name)
            print(f'\n\t=== Sample: {sample_name} ===\n')
            yield from bp.count(dets, num=20)

            yield from bps.mv(piezo.x, x + 500)
            sample_name = name_fmt.format(sample=name, pos = 'pos5', angle='%3.2f'%0.14, wax = '%2.1f'%wa)
            sample_id(user_name='EH', sample_name=sample_name)
            print(f'\n\t=== Sample: {sample_name} ===\n')
            yield from bp.count(dets, num=20)

            yield from bps.mv(piezo.x, x + 250)
            sample_name = name_fmt.format(sample=name, pos = 'pos6', angle='%3.2f'%0.14, wax = '%2.1f'%wa)
            sample_id(user_name='EH', sample_name=sample_name)
            print(f'\n\t=== Sample: {sample_name} ===\n')
            yield from bp.count(dets, num=20)
        ...
        for wa in wa_ran:
            ...
            yield from bps.mv(piezo.x, x - 250)
            sample_name = name_fmt.format(sample=name, pos = 'pos1', angle='%3.2f'%0.30, wax = '%2.1f'%wa)
            sample_id(user_name='EH', sample_name=sample_name)
            print(f'\n\t=== Sample: {sample_name} ===\n')
            yield from bp.count(dets, num=20)

            yield from bps.mv(piezo.x, x - 500)
            sample_name = name_fmt.format(sample=name, pos = 'pos2', angle='%3.2f'%0.30, wax = '%2.1f'%wa)
            sample_id(user_name='EH', sample_name=sample_name)
            print(f'\n\t=== Sample: {sample_name} ===\n')
            yield from bp.count(dets, num=20)

            yield from bps.mv(piezo.x, x - 750)
            sample_name = name_fmt.format(sample=name, pos = 'pos3', angle='%3.2f'%0.30, wax = '%2.1f'%wa)
            sample_id(user_name='EH', sample_name=sample_name)
            print(f'\n\t=== Sample: {sample_name} ===\n')
            yield from bp.count(dets, num=20)

            yield from bps.mv(piezo.x, x - 1000)
            sample_name = name_fmt.format(sample=name, pos = 'pos4', angle='%3.2f'%0.30, wax = '%2.1f'%wa)
            sample_id(user_name='EH', sample_name=sample_name)
            print(f'\n\t=== Sample: {sample_name} ===\n')
            yield from bp.count(dets, num=20)

            yield from bps.mv(piezo.x, x - 1250)
            sample_name = name_fmt.format(sample=name, pos = 'pos5', angle='%3.2f'%0.30, wax = '%2.1f'%wa)
            sample_id(user_name='EH', sample_name=sample_name)
            print(f'\n\t=== Sample: {sample_name} ===\n')
            yield from bp.count(dets, num=20)

            yield from bps.mv(piezo.x, x - 1500)
            sample_name = name_fmt.format(sample=name, pos = 'pos6', angle='%3.2f'%0.30, wax = '%2.1f'%wa)
            sample_id(user_name='EH', sample_name=sample_name)
            print(f'\n\t=== Sample: {sample_name} ===\n')
            yield from bp.count(dets, num=20)
