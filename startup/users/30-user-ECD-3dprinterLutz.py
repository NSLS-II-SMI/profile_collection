def sample_bar(meas_t = 1):
    ...
    x_list = [-57000, -51000, -45000, -39000, -33000, -27000, -21000, -15000, -9000,
    -3000, 3000, 9000, 15000, 21000, 27000, 33000, 39000, 45000, 51000,
    57000, -57000, -51000, -45000, -39000, -33000, -27000, -21000, -15000, -9000,
    -3000, 3000, 9000, 15000, 21000, 27000, 33000, 39000, 45000, 51000]

    y_list = [-800, -800, -800, -800, -800, -800, -800, -800, -800,
    -800, -800, -700, -700, -600, -500, -500, -500, -300, -300,
    -100, 3200, 3200, 3200, 3500, 3500, 3500, 3500, 3500, 3500,
    3500, 3500, 3500, 3500, 3500, 3500, 3500, 3800, 3800]

    y_range = [[0,2000,101],[0,2000,101],[0,2000,101],[0,2000,101],[0,2000,101],[0,2000,101],[0,2000,101],[0,2000,101],[0,2000,101],
    [0,2000,101],[0,2000,101],[0,2000,101],[0,2000,101],[0,2000,101],[0,2000,101],[0,2000,101],[0,2000,101],[0,2000,101],[0,2000,101],
    [0,2000,101],[0,2000,101],[0,2000,101],[0,2000,101], [0,1500,151],[0,1500,151],[0,1500,151],[0,1500,151],[0,1500,151],[0,1500,151],
    [0,1500,151],[0,1500,151],[0,1500,151],[0,1500,151],[0,1500,151],[0,1500,151],[0,1500,151],[0,1500,151],[0,1500,151],[0,1500,151],
    [0,1500,151],
    ]
...
def track_printer_timeRes(exp_t=.1, meas_t=8, trigger_num = 1):
    ...
    while monitor_pv.get() == 1:
        if trigger_signal_pv.get() == 1: # trigger signal to execute
            ...
            #initial dynamics
            yield from data_acquisition(exp_t, meas_t) #controlled by input to track_printer_timeRes
...
def ex_situ(meas_t = 1):
    ...
    for wa in waxs_arc:
        yield from bps.mv(waxs, wa)  
        for i, (x, y) in enumerate(zip(np.linspace(x0, x20, 20), np.linspace(y0, y20, 20))):
            yield from bps.mv(piezo.x, x)
            yield from bps.mv(piezo.y, y)
            sample_name = name_fmt.format(sample=sample, po = '%2.2d'%(1+i), waxs='%2.1f'%wa)
            sample_id(user_name='ED', sample_name=sample_name) 
            print(f'\n\t=== Sample: {sample_name} ===\n')
        ...
        for j, (x, y) in enumerate(zip(np.linspace(x0, x20, 20), np.linspace(y0, y20, 20))):
            ...
            sample_name = name_fmt.format(sample=sample, po = '%2.2d'%(2+i+j), waxs='%2.1f'%wa)
            sample_id(user_name='ED', sample_name=sample_name) 
            print(f'\n\t=== Sample: {sample_name} ===\n')
            yield from bp.rel_scan(dets, piezo.y, *y_range)
            plt.close('all')
...
def ex_situ_printer_height_profile(meas_t = 1):
    ...
    sample_list = ['11_5_hp01', '11_5_hp02', '11_5_hp03', '11_5_hp04', '11_5_hp05', '11_5_hp06', '11_5_hp07', '11_5_hp08', '11_5_hp09', '11_5_hp10', '11_5_hp11', '11_5_hp012']
